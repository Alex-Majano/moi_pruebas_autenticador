  ...
  <div class="card">
    <Toolbar class="py-6 text-center text-2xl">
      <template #start>
        <div>
          <h2 class="font-bold">
            Administrar permisos de acceso al sistema "{{ usuario }}"
          </h2>
        </div>
      </template>
    </Toolbar>
    ...
    <div class="flex justify-center p-6 sm:justify-end">
      <button-component
...
} from '@/services/users/permisosServices.js';
...
const idUsuario = ref(route.params?.id);
const usuario = ref(null);
const permisosModulos = ref([]);

const getPermisosUsuario = async () => {
  loaderPlugin.show();
  const { data } = await getPermisosModulosUsuario(idUsuario.value);
  usuario.value = data.user;
  permisosModulos.value = data.permisos;
  loaderPlugin.hide();
};

const updatePermisosUsuario = async () => {
  loaderPlugin.show();
  ...
  await updatePermissionsUsuario(data);
  loaderPlugin.hide();
